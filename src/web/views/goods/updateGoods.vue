...
<script>
    ...
    let regContent = /http:\S*\w/gi;//验证图文详情中图片
    let TSData = {};//暂存数据
    export default {
        name: "updateGoods",
        components: {
        ...
        data() {
            return {
                ...
                /* 第二页 */
                "oneClassifyId": false,//是否展示规格，【展示：true，隐藏：false】
                ...
                "bannerImg": [],
                "zancunData": {},//详情规格数据暂存
                "zancunData1": {},//详情规格数据暂存 -- 规格是否更改时用
                "skuIsChange": 1,//规格是否更改：1：是 0：否 ,

                /* 第三页 */
                "goodsId": '',//商品id -- 添加至第三步需用id获取规格设置列表
        ...
        methods: {
             ...
             */
            choSku() {
                console.log(this.skuId,this.zancunData1);
                if (this.skuId == '') {
            ...
            goThree() {
                ...
                console.log(this.bannerImg,this.skuTemplateReq,this.zancunData1);
                ...
                //判断是否改动过规格部分
                // let SS = this.skuTemplateReq.skuItemList;
                // console.log(TSData.id, this.skuTemplateReq.id);
                // if (TSData.id != this.skuTemplateReq.id) {
                //     this.skuIsChange = 1
                // } else {
                //     TSData.skuItemList.forEach((ele, i) => {
                //         if (ele.id !== SS[i].id) {
                //             this.skuIsChange = 1
                //         } else {
                //             SS[i].skuSecondItemList
                //         }
                //     })
                // }
                ...
                this.$confirm("此操作将修改该商品,需重新设置规格库存，是否继续 ?", "提示", {
                    "confirmButtonText": "确定",
                    "cancelButtonText": "取消",
                    "type": "warning",
                }).then(() => {
                    this.isSubmit = this.skuOne = this.skuTwo = this.skuThree = true;
                    Goods.updateGoodsPT(postData).then(res => {
                        console.log(res);
                        if (res.code == 200) {
                            this.$message('修改成功');
                            this.threePage = !this.threePage;
                    ...
                    this.$message({center: true, type: "info", message: "已取消".concat('修改').concat("操作")});
                });
            },
            /**
             * 外部图上传成功
            ...
            isPrice(val, e) {
                ...
                if (e == 1) {
                    ...
                    if (!reg.test(val)) {
                        ...
                        this.$message("请您输入正确的金额（只包含二位小数的正整数）");
            ...
            selectList(itemId) {
                ...
                return Array.from(this.skuDetailList, (item, index) => {
                    console.log(item);
            ...
            getDetailPT() {
                Goods.goodsDetailPT({"goodsId": this.goodsId}).then(res => {
                    ...
                    if (res.code == 200) {
                        var list = res.data;
                        this.zancunData = res.data.skuTemplateReq;
                        TSData = res.data.skuTemplateReq;
                        console.log(this.zancunData1);
                        this.oneClassifyId = list.cidOne;
                        ...
                        this.content = list.goodsDetails.match(regContent);
                        console.log(this.content);
                        this.content.forEach(ele => {
                            this.detailImg.push({url: ele});
                        });
                        if (list.skuTemplateReq) {
                            this.skuId = list.skuTemplateReq.id;
                        }
                        ...
                        if(this.skuId){
                            ...
                            Sku.standardListBySkuId({"templateId": this.skuId}).then(res => {
                                ...
                                if (res.code == 200) {
                                    ...
                                    this.zancunData.skuItemList.forEach((ele, i) => {
                                        let check = [];
                                        ele.skuSecondItemList.forEach((val, v) => {
                                            check.push(val.id);
                                        });
                                        this.skuDetailListDefault.splice(i, 1, {
                                            id: ele.id,
            ...
            getDetailPTh() {
                Goods.goodsDetailPTh({"goodsId": this.goodsId}).then(res => {
                    if (res.code == 200) {
                        ...
                        this.getSpanArr(res.data.goodsSkuList);
