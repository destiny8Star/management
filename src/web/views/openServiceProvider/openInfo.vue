                    ...
                    <div class="detail-box">
                        <el-card class="box-card">
                            <div class="item" v-for="(ele,i) in infoList" :key="i">
                                <div>{{ele.text}}：</div>
                                <p v-show="ele.id!=12">{{ele.val}}
                                    <span v-show="ele.id==7">元</span>
                                    <span v-show="ele.id==13">确认到账</span>
                                </p>
                            ...
                            <div class="btn">
                                <el-button v-show="isOpen==0" type="primary" @click="confirm">确认开通账户</el-button>
                            </div>
                        </el-card>
                    </div>
                </div>
...
<script>
    import { OpenServiceProvider, Common, Tags } from "@/web/module/lending";
    export default {
        name: "openInfo",
        data(){
            return {
        ...
        methods:{
            getData(){
                ...
                Common.partnerInfo({"id":this.id}).then(res=>{
                    ...
                    if(res.code==200){
                        ...
                        this.infoList[12].val = res.data.financialAccount;//财务人员账号
            ...
            confirm(){
                const that = this;
                this.$confirm('此操作将确认开通该服务商账户, 是否继续?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    console.log('确认');
                    OpenServiceProvider.isOpen({"id":that.id}).then(res=>{
                        console.log(res);
                        if(res.code==200){
                            this.$message({
                                type: 'success',
                            ...
                            this.$router.push('/openServiceProvider')
                        }else if(res.code==100070 || res.code == 404) {
