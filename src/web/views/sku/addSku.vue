            ...
            <el-main>
                <div class="content">
                    ...
                    <div class="bindClassify">
                        <div class="row">
                            <p>绑定商品分类：</p>
                            <el-select v-model="goodsClassifyId" placeholder="请选择商品分类" @change="choGoodsClassify">
                                <el-option
                        ...
                        <div class="row">
                            <p>商品类型名称：</p>
                            <el-input v-model="skuName" @blur="isName" maxlength="12" clearable placeholder="请输入商品类型名称"></el-input>
                        </div>
                    </div>
                    <div class="skuBox">
                        <el-card class="box-card" v-for="(ele,i) in templateList" :key="i">
                            <div slot="header" class="clearfix">
                                <div class="standard">
                                    ...
                                    <el-input v-model="ele.templateName" maxlength="12" clearable placeholder="请输入规格名称"></el-input>
                                </div>
                                <div class="btnBox">
                                    <el-button :disabled="templateList.length==1" type="danger" @click="delStandard(ele,i)">删除</el-button>
                                </div>
                            </div>
                            <p style="margin-bottom: 6px">规格属性：</p>
                            <p class="item" v-for="(val,v) in ele.skuSecondItemList" :key="v">{{val.templateName}}
                                <el-button @click="delAttr(i,v)" class="delBtn el-icon-circle-close" type="text"></el-button>
                            </p>
                            <el-row style="margin-left: 3px">
                                <el-input v-model="ele.attrText" maxlength="12" placeholder="请输入规格属性" @keydown.13="addAttr(ele,i)" ></el-input>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // 验证sku名称
            isName(){
                Sku.skuName({"id":0,"name":this.skuName}).then(res=>{
                    console.log(res);
                    if(res.code==200){
            ...
            save() {
                ...
                console.log(postData);
                this.$confirm("此操作将添加该模板 , 是否继续 ?", "提示", {
                    "confirmButtonText": "确定",
                    "cancelButtonText": "取消",
                    "type": "warning",
                }).then(() => {
                    Sku.skuAdd(postData).then(res=>{
                        console.log(res);
                        if(res.code==200){
                            this.$message('添加成功');
                            this.isSubmit = false;
